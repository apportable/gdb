...
#define PTRACE_ARG3_TYPE /* XXX long */ void *
  ...
  if (pid == 0)
    {
      ...
      signal (/* XXX __SIGRTMIN */ 32 + 1, SIG_DFL);
  ...
  while (1)
      ...
      if (using_threads)
	{
	  ...
	  /* FIXME drow/2002-06-09: Get signal numbers from the inferior's
	     thread library?  */
	  if (WIFSTOPPED (wstat)
	      && (WSTOPSIG (wstat) == /* XXX __SIGRTMIN */ 32
		  || WSTOPSIG (wstat) == /* XXX __SIGRTMIN */ 32 + 1))
	    {
	      if (debug_threads)
...
linux_init_signals ()
{
  ...
  signal (/* XXX __SIGRTMIN */ 32+1, SIG_IGN);
}
