	       ...
	       int default_line, struct linespec_result *canonical)
{
  ...
  /* Avoid calling symtab_from_filename if we know can,
     it can be expensive.
     FIXME: A safer test is just (*p != '\0'), but that won't catch
     foo::bar.  The problem with the current test is it's unknown, sigh,
     what this will break.  It feels safe enough for now.
     The only known things this might break are DOS path names and
     ObjectiveC, which we don't worry about for now.  */
  if (p[0] == ':' && p[1] != ':')
    {
...
/* Same as decode_variable but only look in OBJFILE.  */

struct symtabs_and_lines
decode_variable_in_objfile (char *copy, int funfirstline,
			    struct linespec_result *canonical,
			    struct objfile *objfile)
{
  ...
  sym = lookup_global_symbol_from_objfile (objfile, copy, VAR_DOMAIN);
  ...
  msymbol = lookup_minimal_symbol (copy, NULL, objfile);
