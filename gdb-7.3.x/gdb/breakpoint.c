		       ...
		       int objfiles_final)
{
  ...
  b->objfiles_final = objfiles_final; /* temp hack */
  if (addr_string)
...
expand_line_sal_maybe (struct symtab_and_line sal, int objfiles_final)
{
  ...
  expanded = expand_line_sal (sal, objfiles_final);
...
{
  int i;
  int objfiles_final = 0;

  /* Temp hack until a more general facility is available.
     If this is "main" mark the breakpoint as having its objfiles
     finalized.  */
  if (sals.nelts == 1)
    {
      ...
      if (sal.symtab != NULL
	  && sal.symtab->objfile == symfile_objfile
	  && canonical->canonical[0]
	  && strcmp (canonical->canonical[0], main_name()) == 0)
	{
	  objfiles_final = 1;
	}
    }
  ...
  for (i = 0; i < sals.nelts; ++i)
    {
      struct symtabs_and_lines expanded = 
	expand_line_sal_maybe (sals.sals[i], objfiles_final);

      create_breakpoint_sal (gdbarch, expanded, canonical->canonical[i],
			     cond_string, type, disposition,
			     thread, task, ignore_count, ops,
			     from_tty, enabled, internal,
			     canonical->special_display, objfiles_final);
    }
}
...
addr_string_to_sals (struct breakpoint *b, char *addr_string, int *found)
{
  ...
  TRY_CATCH (e, RETURN_MASK_ERROR)
	...
	{
	  /* Temp hack to speed up simple cases of "running to main".
	     What we want, I think, is an expansion of linespecs to allow
	     one to more precisely say where to look.
	     E.g. "b main" could be a short hand way of saying
	     "b $main;main" where "$main;" indicates "the main objfile".
	     Or something along those lines.  */
	  if (b->objfiles_final
	      && symfile_objfile != NULL)
	    {
	      sals = decode_variable_in_objfile (s, 1, NULL,
						 symfile_objfile);
	    }
	  else
	    {
...
re_set_breakpoint (struct breakpoint *b)
{
  ...
  if (found)
    {
      make_cleanup (xfree, sals.sals);
      expanded = expand_line_sal_maybe (sals.sals[0], b->objfiles_final);
    }
