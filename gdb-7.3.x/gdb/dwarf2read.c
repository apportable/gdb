...
dwarf2_read_index (struct objfile *objfile)
{
  ...
  if (!dwarf2_use_gdb_index) /* GOOGLE LOCAL */
    return 0;
...
create_debug_types_hash_table (struct objfile *objfile)
{
  ...
  end_ptr = dwarf2_per_objfile->types.buffer + dwarf2_per_objfile->types.size;
