...
/* Return non-zero if we want to log the printing of TYPE.  */

static int
log_type_p (struct type *type)
{
  /* Be massively conservative here and throughout.
     type shouldn't be NULL, but play it safe.  */
  if (type == NULL)
    return 0;
  ...
  switch (TYPE_CODE (type))
    ...
    case TYPE_CODE_PTR:
    case TYPE_CODE_REF:
      return log_type_p (TYPE_TARGET_TYPE (type));
    case TYPE_CODE_TYPEDEF:
      return log_type_p (check_typedef (type));
...
/* Subroutine of log_type to simplify it.  */

static void
emit_printing_type (const char *attr, const char *type_name, int found)
{
  usage_log_printf ("printing-type (%s p-p): %s%s",
		    found ? "have" : "no", attr, type_name);
}

/* Log TYPE if it is interesting (one we might want a pretty-printer for).
   While IWBN to log a more complete type, it's not clear it's necessary
   at this point.  GDB has code for printing types but it uses QUIT, watches
   for line lengths, etc., which complicates things.
   For now try to get by with something simple, and see what data we get.

   ATTR is some aspect of the type that might be useful (e.g. a pointer).
   We don't accumulate attributes though.  */

static void
log_type (struct type *type, const char *attr, int found)
{
  switch (TYPE_CODE (type))
    {
    case TYPE_CODE_MEMBERPTR:
      log_type (TYPE_TARGET_TYPE (type), "member-pointer-to; ", found);
      break;
    case TYPE_CODE_PTR:
      log_type (TYPE_TARGET_TYPE (type), "pointer-to; ", found);
      break;
    case TYPE_CODE_REF:
      log_type (TYPE_TARGET_TYPE (type), "reference-to; ", found);
      break;
    case TYPE_CODE_TYPEDEF:
      {
	/* Try to print the typedef's name.  */
	const char *type_name = type_name_no_tag (type);
	if (type_name != NULL)
	  emit_printing_type (attr, type_name, found);
	else
	  log_type (TYPE_TARGET_TYPE (type), "typedef; ", found);
	break;
      }
    case TYPE_CODE_STRUCT:
    case TYPE_CODE_UNION:
...
/* Log the type of value if it is interesting.
   FOUND is no-zero if a pretty-printer for VALUE was found.  */

static void
maybe_log_value_type (PyObject *value, int found)
{
  /* We found a pretty-printer for this type.
     Log the type name so we can see what types are being printed.  */
  struct value *v = value_object_to_value (value);
  struct type *t = value_type (v);
...
static PyObject *
find_pretty_printer_1 (PyObject *value)
{
  PyObject *function;
...
find_pretty_printer (PyObject *value)
{
  ...
  function = find_pretty_printer_1 (value);

#ifdef HAVE_GOOGLEISMS
  maybe_log_value_type (value, function != Py_None);
#endif
